...
/*!\file MemoryPool.h
 * \brief Contains the "MemoryPool" Class-defintion.
 */

#ifndef __INC_MemoryPool_h__
#define __INC_MemoryPool_h__
...
static const std::size_t DEFAULT_MEMORY_POOL_SIZE        = 50000000 ;                          //!< Initial MemoryPool size (in Bytes)
static const std::size_t DEFAULT_MEMORY_CHUNK_SIZE       = 128 ;                           //!< Default MemoryChunkSize (in Bytes)	
static const std::size_t DEFAULT_MEMORY_SIZE_TO_ALLOCATE = DEFAULT_MEMORY_POOL_SIZE ; //!< Default Minimal Memory-Size (in Bytes) to Allocate. 
 ...
 */
class MemoryPool : public IMemoryBlock
{
  public :	  
    /*!
	Contructor
	...
	*/
	...
	/*!
	  ...
	  \param [IN] sMemorySize       Sizes (in Bytes) of Memory.
	*/
	void FreeMemory(void *ptrMemoryBlock, const std::size_t &sMemoryBlockSize=1) ;
    void FreeMemory(SMemoryChunk *ptrChunk);
  ...
  private :
	...
	*/
	...
	static void FreeAllAllocatedMemory() ; //!< Will free all Aloocated Memory to the Operating-System again.
	...
	static void DeallocateAllChunks() ; //!< Deallocates all Memory needed by the Chunks back to the Operating-System.
	...
	bool RecalcChunkMemorySizePart(SMemoryChunk *ptrChunk, unsigned int uiChunkCount, size_t ChunksMemoryBlockSize);
    ...
    static SMemoryChunk *m_ptrFirstChunk ;  //!< Pointer to the first Chunk in the Linked-List of Memory Chunks
    static SMemoryChunk *m_ptrLastChunk ;   //!< Pointer to the last Chunk in the Linked-List of Memory Chunks
	...
	//static MemPool::MemoryPool *GlobalPool;
